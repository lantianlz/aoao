...
class UserBase(object):
    ...
    def send_confirm_email(self, user):
        ...
        if not cache_obj.get_time_is_locked(key, 60):
            ...
            async_send_email(user.email, u'嗷嗷洗车邮箱验证', utils.render_email_template('email/verify_email.html', context), 'html')
    ...
    def send_forget_password_email(self, email):
        ...
        if not cache_obj.get_time_is_locked(key, 60):
            ...
            async_send_email(email, u'嗷嗷洗车找回密码', utils.render_email_template('email/reset_password.html', context), 'html')
        return 0, dict_err.get(0)
    ...
    def get_user_by_external_info(self, source, access_token, external_user_id,
        ...
        if et:
            ...
            email = '%s_%s@mraoaoxc.com' % (source, int(time.time() * 1000))
